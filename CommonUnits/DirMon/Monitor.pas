...
type
  ...
  protected
    ...
    procedure DoChange(Action: TDirectoryMonitorAction; const FileName: WideString); virtual;
...
var
  WaitResult, BytesRead, WStrLen: DWORD;
...
procedure TDirectoryMonitor.DoChange(Action: TDirectoryMonitorAction; const FileName: WideString);
begin
...
var
  Action: TDirectoryMonitorAction;
  FileNamePtr: PWideChar;
begin
  if Msg.Msg = WMUSER_DIRECTORYCHANGED then
  begin
    Action := TDirectoryMonitorAction(Msg.WPARAM);
    FileNamePtr := PWideChar(Msg.LPARAM);
    DoChange(Action, FileNamePtr);
    StrDispose(FileNamePtr);
  end;
end;
