...
uses
  ...
  System.SysUtils, System.Variants, System.Classes, System.ImageList, System.Actions, System.Generics.Collections,
  ...
  UnitListViewSort, UnitBaseMTP, UnitTripDefs, UnitTripObjects, UnitGpxDefs, UnitGpxObjects, UnitGpi, UnitExplore,
...
const
  ...
  BgExploreCaption        = 'Explore info';
  BgTripInfoCaption       = 'Trip info';
  BgPoiInfoCaption        = 'Poi(Gpi) info';
  BgFitInfoCaption        = 'Course(fit) info';
...
type
  ...
  TFrmTripManager = class(TForm)
    ...
    ProcessTrackLogs1: TMenuItem;
    PopupPostProcess: TPopupMenu;
    ...
    procedure TripGpiFitExplTimerTimer(Sender: TObject);
  ...
  private
    ...
    AnExploreList: TObjectlist<TExpl_Object>;
...
uses
  System.StrUtils, System.UITypes, System.DateUtils, System.TypInfo, System.IOUtils, System.JSON,
  Winapi.ShellAPI,
  Vcl.Clipbrd,
  UnitGarminDevice, UnitProcessOptions, UnitRegistry, UnitRegistryKeys, UnitStringUtils, UnitSqlite, UnitOSMMap,
  UnitGeoCode, UnitVerySimpleXml, UnitRedirect, UnitGpxTripCompare, UnitModelConv, UnitVehProfile,
...
procedure TFrmTripManager.BgTripInfoClick(Sender: TObject);
begin
...
begin
  ...
  ATripList.ForceRecalc;
  ATripList.SaveToFile(FrmTripEditor.CurFile);
  if (FrmTripEditor.CurIsDeviceFile) then
    FrmTripEditor.SendDeviceFile(0, FrmTripEditor.CurFile)
  else
    BtnRefreshFileSysClick(Sender);
end;
...
function TFrmTripManager.HasExploreData(const ExploreType: integer = -1): boolean;
begin
  result := (BgTripInfo.ItemIndex = 1) and
            (CdsExploreDb.State in [dsBrowse]) and
            (TvTrip.Selected <> nil) and
            (TvTrip.Selected.Data <> nil) and
            (TObject(TvTrip.Selected.Data) is TExpl_Object) and
            ((ExploreType = -1) or (TExpl_Object(TvTrip.Selected.Data).Expl_Type = ExploreType));
end;
...
procedure TFrmTripManager.BtnPostProcessMouseUp(Sender: TObject;
...
begin
  if (PnlTripInfo.Width < TvTrip.Width + VSplitterTree_Grid.Width + VlTripInfo.Width) then
  begin
    PnlHideGrid.Caption := '<';
    PnlTripInfo.Width := TvTrip.Width + VSplitterTree_Grid.Width + VlTripInfo.Width;
  end
  else
  begin
    PnlHideGrid.Caption := '>';
    PnlTripInfo.Width := TvTrip.Width;
  end;
end;
...
procedure TFrmTripManager.PnlTripInfoResize(Sender: TObject);
begin
  SbPostProcess.Panels[0].Width := PnlTripInfo.Width div 2;
  SbPostProcess.Panels[1].Width := PnlTripInfo.Width div 2;
end;
...
begin
  ...
  SaveGPX1.Enabled := ( MnuCompareGpxRoute.Enabled ) or
                      ...
                      ( (AFitInfo <> nil)  and (AFitInfo.Count > 0)) or
                      ( (BgTripInfo.ItemIndex = 1) and (CdsExploreDb.RecordCount > 0));
...
begin
  ...
  if not USBEvent.RegisterUSBHandler(GUID_DEVINTERFACE_WPD, Handle, WM_USBCONNECTEVENT) then
...
{$ENDIF}
  ...
  AnExploreList := TObjectList<TExpl_Object>.Create(true);
...
begin
  if (PnlTripInfo.Width = TvTrip.Width) then
    PnlHideGrid.Caption := '>'
  else
    PnlHideGrid.Caption := '<';
...
begin
  ...
  FSavedFolderId := CurrentDevice.PathId[APath];
end;
...
procedure TFrmTripManager.TripGpiFitExplTimerTimer(Sender: TObject);
begin
  ...
  case BgTripInfo.ItemIndex of
    0:
      begin
        if (LstFiles.Selected = nil) or
           (TBase_Data(LstFiles.Selected.Data).IsFolder) then
          exit;
...
var
  ...
  procedure AddExplore(AnExplore: TExpl_Object);
  var
    AField: TField;
      ...
      begin
        if (SameText(AField.FieldName, 'TRACK_POINTS')) then
          VlTripInfo.Strings.AddPair(AField.FieldName, 'Binary')
                ...
                (SameText(AField.FieldName, 'ROUTE_POINTS')) then
        begin
          VlTripInfo.Strings.AddPair(AField.FieldName, 'JSON');
          Expl_ParseJson(AField.AsString, AStringList);
          VlTripInfo.Strings.AddStrings(AStringList);
        end
        else
          VlTripInfo.Strings.AddPair(AField.FieldName, AField.DisplayText);
      end;
    finally
      AStringList.Free;
    end;
...
// Set Editmode of VlTripInfo
    ...
    begin
      ...
      if (AnItem = nil) then
      begin
        if (VlTripInfo.Cells[0, Index +1] = 'METADATA') or
           (VlTripInfo.Cells[0, Index +1] = 'ROUTE_POINTS') then
        begin
          VlTripInfo.ItemProps[Index].EditStyle := esEllipsis;
          continue;
        end;
  ...
  begin
    ...
    begin
      ...
      UnlockFiles := UnlockFiles or
                     (CurrentDevice.FileId[FSavedFolderId, ChangeFileExt(ANitem.Caption, UnlExtension)] <> '');
    end;
    Inc(SelCount);
  end;
...
begin
  CheckDevice;
  CurrentDevice.TransferExistingFile(LocalFile, FSavedFolderId, AListItem);
...
begin
  ...
  try
    ...
    begin
      ...
      FSavedParent := CurrentDevice.ReadFiles(LstFiles.Items, SParent, FCurrentPath);
...
procedure TFrmTripManager.SetupBgTripInfo(const Caption: string);
var
  BtnCount: integer;
begin
  BtnCount := 1;
  if (FileExists(GetDeviceTmp + ExploreDb)) then
    Inc(BtnCount);
  if (BgTripInfo.Items.Count <> BtnCount) then
  begin
    BgTripInfo.Items.Clear;
    while (BgTripInfo.Items.Count < BtnCount) do
      BgTripInfo.Items.Add;
  end;
  BgTripInfo.Items[0].Caption := Caption;
  BgTripInfo.ItemIndex := 0;
  if (BtnCount > 1) then
...
begin
  ...
  if (Assigned(APOIGroupList)) then
    APOIGroupList.Clear;
  if (Assigned(AnExploreList)) then
    AnExploreList.Clear;
      ...
      begin
        ...
        begin
          ...
          if (SameText(CdsExploreDb.FieldByName('name').AsString, SelectNode)) then
            Node2Select := TvTrip.Items.AddChildObject(ExplGroupNode, CdsExploreDb.FieldByName('name').AsString, AExpl_Object)
          else
            TvTrip.Items.AddChildObject(ExplGroupNode, CdsExploreDb.FieldByName('name').AsString, AExpl_Object);
          ExplGroupNode.Expand(false);
        end;
        CdsExploreDb.Next;
      end;
    finally
...
begin
  ...
  SaveControlSize(Reg_SavedLayout, PnlTripInfo);
...
begin
  ...
  begin
    ...
    ReadControlSize(Reg_SavedLayout, PnlTripInfo);
...
procedure TFrmTripManager.WMUSBConnectEvent(var Msg: TMessage);
var
